...
//dataSource
@property(nonatomic, strong)kDataSource *arrDataSource;
...
- (void)viewDidLoad {
    ...
    for(int i=0;i<_arr.count;i++){
        TestModal *m=[[TestModal alloc]init];
        m.discribe =_arr[i];
        m.title=_arr[i];
        //把模型那存到模型数组中
    ...
    TableViewCellConfigureBlock configureCell = ^(TestTableViewCell *cell,TestModal *modal){
        cell.modal = modal;
    };
    self.arrDataSource = [[kDataSource alloc]initWithtableViewWithRow:[self.arrModel copy] cellIdentifier:testcell configureCellBlock:configureCell];
    self.tableView.dataSource = self.arrDataSource;
